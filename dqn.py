...
NUM_ENVS = 4
LR = 2.5e-4
SAVE_PATH = './atari_dqn_model.pack'
SAVE_INTERVAL = 10000
LOG_DIR = './logs/atari_dqn'
LOG_INTERVAL = 1000


def nature_cnn(observtion_space, depths=(32,64,64), final_later=512):
    n_input_channels = observtion_space.shape[0]
    ...
    cnn = nn.Sequential(
        ...
        nn.ReLU(),
        nn.Flatten()
        # ,nn.Linear(depths[2]*7*7, final_later),
        # nn.ReLU()
    )

    with torch.no_grad():
        n_flatten = cnn(torch.as_tensor(observtion_space.sample()[None]).float()).shape[1]
    
    out = nn.Sequential(cnn, nn.Linear(n_flatten, final_later), nn.ReLU())
...
# make_env = lambda:make_atari_deepmind('Breakout-v4')

device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
print(f"Using device: {device}")
make_env = lambda: Monitor(make_atari_deepmind('Breakout-v4'), allow_early_resets=True)
